...
//    _mini_treeprint(_mini_root);
    if ((_mini_root!=NULL)) {
        ...
        return _mini_concatLists(_mini_concatLists(_mini_postOrder(_mini_root->_mini_left),
        _mini_postOrder(_mini_root->_mini_right)), _mini_temp);
    }
    return NULL;
}
...
//    printf("Size of _mini_node: %ld\n", sizeof(struct _mini_node));
//    printf("Size of _mini_tnode: %ld\n", sizeof(struct _mini_tnode));
    while ((_mini_i<10L)) {
//        printf("Here %d\n", _mini_i);
